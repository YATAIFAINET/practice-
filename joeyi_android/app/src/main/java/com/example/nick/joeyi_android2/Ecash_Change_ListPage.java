...
public class Ecash_Change_ListPage extends Activity {
    ...
    public void Get_Exchange_thread() {
        mThread = new HandlerThread("get_ecash_to_money_list");
        mThread.start();
        mThreadHandler = new Handler(mThread.getLooper());
        mThreadHandler.post(new Runnable() {
            public void run() {
                final String jsonString = executeQuery("get_ecash_to_money_list");
    ...
    public final void newlist(String input) throws JSONException {
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            item.put("ps", jsonArray.getJSONObject(i).getString("ps"));
            items.add(item);
        }
        SimpleAdapter simpleAdapter = new SimpleAdapter(this,
                items, R.layout.ecash_change_list, new String[]{"date", "point", "ps"},
                new int[]{R.id.date, R.id.point, R.id.ps });
